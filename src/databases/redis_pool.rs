...
impl From<RedisConnectionManager> for SessionRedisPool {
    fn from(manager: RedisConnectionManager) -> Self {
        SessionRedisPool { manager }
    }
}
...
impl DatabasePool for SessionRedisPool {
    ...
    async fn count(&self, _table_name: &str) -> Result<i64, SessionError> {
        let mut con = self.manager.aquire().await?;
        let count: i64 = redis::cmd("DBSIZE").query_async(con.as_mut()).await?;
    ...
    async fn load(&self, id: &str, _table_name: &str) -> Result<Option<String>, SessionError> {
        let mut con = self.manager.aquire().await?;
        let result: String = redis::cmd("GET").arg(id).query_async(con.as_mut()).await?;
        Ok(Some(result))
    }

    async fn delete_one_by_id(&self, id: &str, _table_name: &str) -> Result<(), SessionError> {
        let mut con = self.manager.aquire().await?;
        redis::cmd("DEL").arg(id).query_async(con.as_mut()).await?;
        Ok(())
    }
    ...
    async fn delete_all(&self, _table_name: &str) -> Result<(), SessionError> {
        let mut con = self.manager.aquire().await?;
        redis::cmd("FLUSHDB").query_async(con.as_mut()).await?;
        Ok(())
    }
